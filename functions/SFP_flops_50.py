...
def softpruning(f, args, model, thres, end_flag, output_size):
    ...
    for k, m in enumerate(model.modules()):
        if isinstance(m, nn.BatchNorm2d):
            ...
            thre_temp = torch.kthvalue(weight_copy.abs(), int(m.weight.data.shape[0]*0.5))[0].item()
            print('thre_temp:{}'.format(thre_temp))
            if thre<thre_temp:
                thre_temp=thre;
            mask = weight_copy.abs().gt(thre_temp).float().cuda()

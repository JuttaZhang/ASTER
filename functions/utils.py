...
def newthres(model,ratio):
    ...
    '''for i in range(len(y)):
        if abs(y[i]) != 0:
            ...
            break'''
    return thres
def get_hoyper(model):
    hoyper = []
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            l1 = torch.sum(torch.abs(m.weight.data))
            l2 = torch.sum(m.weight.data ** 2) ** 2
            hoyper.append(torch.mul(torch.sign(m.weight.data)*(l1/l2),(torch.full(m.weight.data.size(), l2).cuda()-torch.abs(m.weight.data)*l1)))
    return hoyper
...
def get_bn(model):
    ...
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            ...
            index += size
    return bn
def get_bn_layer(model):
    total = 0  # out_channel数量
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            total += m.weight.data.shape[0]
    bn = torch.zeros(total)  # 把所以bn层的｜weight｜合到同一个tensor中
    index_layer = torch.zeros(total)
    index = 0

    for k, m in enumerate(model.modules()):
        if isinstance(m, nn.BatchNorm2d):
            size = m.weight.data.shape[0]
            bn[index:(index + size)] = m.weight.data.clone()
            index_layer[index:(index + size)] = torch.full(m.weight.data.shape, k)
            index += size
    index_arr = range(0, index)

    return index_arr, bn, index_layer
...
def lbfgs_alpha_showtensorboard(args,model,epoch,tb_writer):
    bn_weights = get_bn(model)
    tb_writer.add_histogram('lbfgs_alpha_0.85_{}_{}_{}'.format(args.depth, args.num, args.s), bn_weights.numpy(), epoch, bins='doane')
...
def hoyper_showtensorboard(args, model, epoch, tb_writer):
    bn_weights = get_bn(model)
    tb_writer.add_histogram('test_hoyper_{}_{}_{}'.format(args.model, args.num, args.s), bn_weights.numpy(), epoch,
                            bins='doane')

...
def get_data_set(args, type='train'):
    if type == 'train':
        return imagenet_dali.get_imagenet_iter_dali('train', args.data, args.train_batch_size,
                                                    num_threads=4, crop=224, device_id=args.gpu, num_gpus=1)
    else:
        return imagenet_dali.get_imagenet_iter_dali('val', args.data, args.eval_batch_size,
                                                    num_threads=4, crop=224, device_id=args.gpu, num_gpus=1)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = get_data_set(args, 'train')
    val_loader = get_data_set(args, 'test')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        end_time = time.time()
        print("evaluate cost: {} seconds.".format((end_time - start_time)*args.epochs))
...
def adjust_learning_rate(args, optimizer: Optimizer, epoch: int, iteration: int, train_loader_len: int, warmup: bool):
    """
    ...
    if epoch >= 120:
        ...
        cur_lr = cos_lr(0.05, epoch=epoch-120, iteration=iteration,

...
#------------------------------------------
#使用传统方法的补充实验，tradeoff = prec+pruning_ratio
#使用的判断实验停止的方法是剪掉给定剪枝率的channel数，如果大于剪枝上限，就剪上限值
#结果存放位置 /results2
#------------------------------------------
...
def train_evaluate(parameterization):
    ...
    filepath = './results2/{}_{}'.format(args.s, parameterization.get("pruning_ratio", 0.7))
    ...
    #model_new
    args.percent = parameterization.get("pruning_ratio", 0.7)
    model = vgg(depth=16, dataset='cifar10')
    model.cuda()
...
best_parameters, values, experiment, model = optimize(
    parameters=[
        ...
        {"name": "pruning_ratio", "type": "range", "bounds": [1e-10, 1-1e-10], "log_scale": True}
    ],
    evaluation_function=train_evaluate,
    objective_name='tradeoff',
    total_trials=10,
)

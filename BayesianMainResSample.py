...
# resnet56,resnet110,cifar10 bayesion,对于数据集进行抽样看一下结果咋样，但是现在存在一个问题就是方差应该会比较大
...
parser.add_argument('--pec', type=float, default=0.5,
                    help='衡量tradeoff的比例')
parser.add_argument('--lb', type=float, default=0.3,#0.3 56   0.25 110
...
def train_evaluate(parameterization):
    ...
    model.cuda()
    thres = 6.511471552067271e-07#parameterization.get("init_thres", 0.0001)
    init_thres = thres
    sample_percent = parameterization.get("percent", 0.1)
    x = 0.03536249898532406#parameterization.get("x", 0.01)
    flops = flops_init
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    filepath = './resultsRes56/{}_{}_{}_{}_{}'.format(args.depth, init_thres, x, sample_percent, args.num)
    ...
    for epoch in range(0, args.epochs):
        ...
        model.train()
        optimizermu = LBFGS(get_bnparameters(model), lr=args.lr, history_size=50, mu=args.s, batch_size=args.batch_size,
        ...
        if (epoch + 1) % 5 == 0 and flag and upper_flag and not chosen_flag:
            ...
            num_sample = len(train_loader)*sample_percent
...
best_parameters, values, experiment, model = optimize(
    parameters=[
        #{"name": "init_thres", "type": "range", "bounds": [1e-8, 0.1], "log_scale": True},
        #{"name": "x", "type": "range", "bounds": [0.0001, 0.1]},
        {"name": "percent", "type": "range", "bounds": [0.00001, 0.999999]},
    ],
